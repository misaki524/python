    ...
    def spawn_enemy(self,left_x,right_x):
        ...
        for tx in range(left_x,right_x+1):
            for ty in range(16):
                x=tx*8
                y=ty*8
                title_type=get_tile_type(x,y)

                if title_type==TILE_SLIME1_POINT:#グリーンスライムの出現位置の時
                    enemies.append(Slime(game,x,y,False))
                elif title_type==TILE_SLIME2_POINT:#レッドスライム出現位置の時
                    enemies.append(Slime(game,x,y,True))
                elif title_type==TILE_MUMMY_POINT:#マミーの出現位置の時
                    enemies.append(Mummy(game,x,y))
                elif title_type==TILE_FLOWER_POINT:#フラワー出現位置の時
